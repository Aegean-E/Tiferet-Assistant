...
class Decider:
    ...
    def _check_concurrency_and_panic(self) -> Optional[Dict[str, Any]]:
        """Check if planning is locked or panic mode is active."""
    ...
    def _gather_structural_metrics(self) -> Dict[str, Any]:
        """Gather memory stats and CRS allocation."""
    ...
    def _get_prioritized_goals(self, limit: int = 5) -> Tuple[List[Any], bool]:
        """Fetch and prioritize active goals. Returns (active_goals, is_tabula_rasa)."""
        ...
        if self.crs:
            ...
            ranked_goals_info = self.crs.prioritize_goals(mapped_goals, 10000)
        ...
        return active_goals[:limit], is_tabula_rasa

    def _check_reflex_actions(self,
                              active_goals: List[Any],
                              necessity_goals: List[Any],
                              gevurah_score: float,
                              hesed_score: float,
                              netzach_signal: str,
                              motivation: float) -> Optional[Dict[str, Any]]:
        """Check for immediate reflex actions. Returns a plan dict if triggered."""
    ...
    def _build_planning_context(self,
                                ...
                                system_mode: str,
                                crs_status: Dict[str, Any],
                                stability_state: Dict[str, Any],
                                allow_daydream: bool) -> str:
        """Construct the context string for the planning LLM."""

        recent_docs = self.document_store.list_documents(limit=5)

        # Fetch Curiosity Gaps
    ...
    def _run_bicameral_dialogue(self, context: str) -> Tuple[str, str]:
        """Generate conflicting internal voices (Impulse & Reason)."""
        settings = self.get_settings()
        ...
        return voice_impulse, voice_reason

    def _execute_decision_llm(self, context: str, options_text: str, voice_impulse: str, voice_reason: str) -> Tuple[str, str]:
        """Execute the main decision LLM call."""
        settings = self.get_settings()
        ...
        return response, internal_thought

    def _process_decision_command(self,
                                  response: str,
                                  response_upper: str,
                                  active_goals: List[Any],
                                  allow_daydream: bool,
                                  just_finished_heavy: bool) -> Dict[str, Any]:
        """Process the parsed command and return the plan."""

        if "[WAIT]" in response_upper:
            return {"task": "wait", "cycles": 0, "reason": "Decider chose to wait"}
            # Do not reset consecutive_daydream_batches on WAIT
            ...
            try:
                ...
                if topic.upper() in bad_topics or not topic:
                    ...
                    current_goals = self.memory_store.get_active_by_type("GOAL")
                    if current_goals:
                            topic = current_goals[0][2]
                    else:
        ...
        return {"task": "wait", "cycles": 0, "reason": "End of decision command processing"}

    def decide(self) -> Dict[str, Any]:
        """Decide what to do next. Returns a plan dict."""
        # 1. Concurrency Check
        concurrency_check = self._check_concurrency_and_panic()
        if concurrency_check:
            return concurrency_check

        try:
            # 2. Gather Metrics & Manage Goals
            metrics_data = self._gather_structural_metrics()
            structural_metrics = metrics_data["structural_metrics"]
            crs_status = metrics_data["crs_status"]
            stability_state = metrics_data["stability_state"]

            system_mode = crs_status.get("system_mode", "EXECUTION")
            allow_goal_creation = crs_status.get("allow_goal_creation", True)

            # 0. Goal Autonomy Cycle
            ...
            # 3. Prioritize Goals
            active_goals, is_tabula_rasa = self._get_prioritized_goals()
            
            # Check for Necessity Goals
            ...
            # 4. Reflex Check
            reflex_decision = self._check_reflex_actions(
                active_goals, necessity_goals, gevurah_score, hesed_score, netzach_signal, motivation
            )
            if reflex_decision:
            ...
            context = self._build_planning_context(
                ...
                gevurah_score=gevurah_score,
                hesed_score=hesed_score,
                netzach_signal=netzach_signal,
                motivation=motivation,
                hod_signal=hod_signal,
                current_utility=current_utility,
                current_friction=current_friction,
                system_mode=system_mode,
                crs_status=crs_status,
                stability_state=stability_state,
                allow_daydream=allow_daydream
            )
            
            # Options Text Logic (Need to replicate)
            ...
            # 6. Bicameral Dialogue
            voice_impulse, voice_reason = self._run_bicameral_dialogue(context)

            # 7. Execute Decision LLM
            response, internal_thought = self._execute_decision_llm(context, options_text, voice_impulse, voice_reason)
            ...
            # 8. Process Command
            return self._process_decision_command(
                response, response_upper, active_goals, allow_daydream, just_finished_heavy
            )
