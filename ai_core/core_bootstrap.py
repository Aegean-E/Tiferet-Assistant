            ...
            self.core.decider = Decider(
                ...
                actions={
                    **self.core.action_manager.get_tools(),
                    ...
                    "LIST_DOCUMENTS": list_documents_wrapper,
                    "LIST_DOCS": list_documents_wrapper,
                    "read_document": read_document_wrapper,
                    "READ_DOCUMENT": read_document_wrapper,
                    "READ_DOC": read_document_wrapper,
                    ...
                    "SIMULATE_COUNTERFACTUAL": lambda p: self.core.daat.run_counterfactual_simulation(p) if self.core.daat else "Daat missing",
                    "SIMULATE": lambda p: self.core.daat.run_counterfactual_simulation(p) if self.core.daat else "Daat missing",
                    "CREATE_GOAL": lambda c: self.core.decider.create_goal(c) if getattr(self.core, 'decider', None) else "Decider missing"
                },
                log_fn=self.core.log,
